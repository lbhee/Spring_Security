...
public class CustomerService {
	...
	public String noticeReg(Notice n, HttpServletRequest request, Principal principal) throws Exception {
	      ...
	      System.out.println("글쓰기 CustomerService");
	       ...
	       * 사용자의 권한 정보가 필요할때
	      //인증된 사용자 정보를 가지고 와서 
	      SecurityContext context = SecurityContextHolder.getContext();// 스프링 시큐리티가 가지는 모든 정보를 얻어오겠다
	      Authentication auth = context.getAuthentication(); //인증에 관련된 것만 (시큐리티가 가지고 있는 getAuthentication이 필요함)
	      
	      UserDetails userinfo = (UserDetails)auth.getPrincipal(); //사용자의 정보를 담고 있는 최소한의 객체
	      //UserDetails 로그인한 사용자의 정보 (^^ 권한 정보)
